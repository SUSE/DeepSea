...
class Matcher(object):
    ...
    def check_stages(self, jid, ret):
        orch_name = "{}{}{}".format(
            bcolors.HEADER, ident.stagename, bcolors.ENDC)
        ...
        if self.stage_ended(ret, jid):
            if 'success' in ret['data']:
                status = "{}succeeded {}".format(
                    bcolors.OKGREEN, bcolors.ENDC) if ret['data']['success'] is True else "{}failed {}".format(bcolors.FAIL, bcolors.ENDC)
    ...
    def construct_message(self, base_type, ret):
        ...
        message = "{}{} {} on {}{}".format(
            prefix, verb, func_name, ' '.join(target), suffix)
        ...
        if not self.is_orch(base_type):
            ...
            if not ident.filters['duplicates']:
                if ident.prev_name != func_name:
                    ...
                    Printer(message)
                    ident.counter += 1
                    logger.debug("Incrementing the counter to {}; still calling {}".format(
                        ident.counter, base_type))
