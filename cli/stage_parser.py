...
class SLSParser(object):
    ...
    @staticmethod
    def parse_state_steps(state_name):
        """
        ...
        for key, steps in state_dict.items():
            if key == 'include':
                for inc in state_dict['include']:
                    ...
                    result.extend(SLSParser.parse_state_steps(include_state_name))
            else:
                if isinstance(steps, dict):
                    for fun, args in steps.items():
                        ...
                        if fun == 'salt.state':
                            ...
                            result.append(state)
                            result.extend(SLSParser.parse_state_steps(state.state))
                            ...
                            if 'fire_event' in builtin.args and builtin.args['fire_event']:
