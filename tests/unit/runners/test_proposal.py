...
class TestProposalRunner(object):
    ...
    def test_query_node_disks(self, mock_client, minions):
        ...
        call1 = call('data1.ceph', 'mine.flush', expr_form='compound')
        call2 = call('data1.ceph', 'mine.update', expr_form='compound')
        call3 = call('data1.ceph', 'cephdisks.list', expr_form='compound')
        RD = self.NoInitReplaceDisk(minion)
        RD.name = 'data1.ceph'
    ...
    @pytest.mark.parametrize("execution_number", range(1, 5))
    @patch('salt.client.LocalClient', autospec=True)
    def test_prepare_device_files(self, mock_client, execution_number, minions):
        minion = minions['minion{}'.format(execution_number)]
        local_client = mock_client.return_value
